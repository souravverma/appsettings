...
describe("Unit Tests for PartzoneswithFunctionalItem", () => {
  const partzoneswithFunctionalItem = new PartzoneswithFunctionalItem();
  

  it("PartzoneswithFunctionalItem should be defined", () => {
    expect(partzoneswithFunctionalItem).toBeDefined();
  });
  
  it("should return partzone with fin", async () => {
    const output = await partzoneswithFunctionalItem.getPartZoneWithIncludedFin([]);
    expect(output).toBeDefined;
  });

  it("should return partzone comparative fin list", async () => {
    const output = partzoneswithFunctionalItem.getFinResponse([],["1234VB"]);
    expect(output).toBeDefined;
  });
});
